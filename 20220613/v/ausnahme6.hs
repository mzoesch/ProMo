...
import qualified Control.Exception as E ( Exception, SomeException, evaluate, throw, try)
...
main :: IO ()
main = do
  ergebnis <- E.try (E.evaluate $ show (2 `div` 0)) :: IO (Either E.SomeException [Char])
  case ergebnis of
